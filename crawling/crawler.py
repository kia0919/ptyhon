...
# 웹 스크래핑 (Scrappint): 웹 페이지의 응답을 받아 데이터를 분석하여 필요한 데이터를 수집하는 방법
...
# pip install beautifulsoup4 selenium(한번에 설치.)
...
time.sleep(2)   # 브라우저가 켜졌다가 5초뒤에 꺼짐.

driver.get(URL)
time.sleep(2)   # 네이버로 이동뒤 3초후에 꺼짐

search_input = driver.find_element(By.ID, 'query')   # 요소 찾기
search_input.send_keys('제네시스')
time.sleep(1)   # 네이버 켜졌다가 검색창이 제네시스가 입력됨
...
for news_title_element in news_title_elements:
    ...
    print(title)    # 제네시스 뉴스 title(제목)들만 출력됨
